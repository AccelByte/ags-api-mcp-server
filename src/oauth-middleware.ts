  ...
  async getAccessToken(): Promise<ClientCredentialsResult | null> {
    ...
    if (this.cachedToken && Date.now() < this.cachedToken.expiresAt) {
      logger.debug(
        ...
        "Using cached client credentials token",
      );
      return {
    ...
    try {
      logger.debug(
        { tokenUrl: this.tokenUrl, clientId: this.clientId },
        "Requesting new client credentials token",
      );
      ...
      if (!access_token) {
        logger.error(
          ...
          "No access token in client credentials response",
        );
        return null;
      }
...
export class OAuthMiddleware {
  ...
  private validateConfig(): void {
    ...
    const missing = required.filter(
      (key) => !this.config[key as keyof OAuthConfig],
    );
    ...
    if (missing.length > 0) {
      ...
      logger.warn(
        "OAuth authentication will be disabled. Set environment variables to enable.",
      );
    }
  }
  ...
  initiateOAuth(req: Request, res: Response): void {
    ...
    if (!this.config.clientId || !this.config.authorizationUrl) {
      logger.error(
        "OAuth not configured - missing clientId or authorizationUrl",
      );
      res.status(501).json({
        error:
          "OAuth not configured. Please set OAUTH_CLIENT_ID and OAUTH_AUTHORIZATION_URL environment variables.",
      });
      return;
    }
    ...
    if (otpToken) {
      ...
      if (!sessionToken) {
        ...
        res.status(400).json({
          ...
          message:
            "The one-time password token is invalid, expired, or already used. Please generate a new one.",
        });
        return;
      }
    ...
    if (!sessionToken) {
      ...
      res.status(400).json({
        ...
        message:
          "OAuth login requires an OTP token. Call the start_oauth_login tool to get a secure login URL.",
      });
      return;
    }
  ...
  async handleCallback(req: Request, res: Response): Promise<void> {
    ...
    if (baseState !== baseStoredState) {
      ...
      if (!storedState) {
        ...
        logger.debug(
          "No stored state found - proceeding with callback (likely external OAuth flow) with localhost validation",
        );
      } else {
    ...
    if (!codeVerifier) {
      ...
      const isDirectBrowserFlow =
        req.get("User-Agent")?.includes("Mozilla") ||
        req.get("Referer")?.includes("accelbyte.io");

      if (isDirectBrowserFlow) {
        logger.error(
          "This appears to be a direct browser OAuth flow, but cookies are missing!",
        );
        logger.error(
          "Without code_verifier, AccelByte will reject the token exchange (invalid_grant)",
        );
        logger.error(
          "Possible causes: 1) Cookies blocked by browser, 2) Cookie sameSite issue, 3) Domain mismatch",
        );

        res.status(500).json({
          error: "OAuth flow failed",
          message:
            "PKCE cookies not preserved. Please check browser cookie settings.",
          details: {
            expectedCookies: ["oauth_state", "oauth_code_verifier"],
      ...
      logger.debug(
        "Code verifier not found in cookies - assuming external OAuth flow",
      );
    }
      ...
      logger.debug(
        {
          ...
          requestData: {
            ...tokenRequestData,
            client_secret: this.config.clientSecret
              ? `***REDACTED*** (${this.config.clientSecret.length} chars)`
              : undefined,
            code: String(code).substring(0, 10) + "...",
            code_verifier: codeVerifier
              ? `***REDACTED*** (${codeVerifier.length} chars)`
              : undefined,
          },
          formEncodedBody: formData
            .toString()
        ...
        logger.error(
          {
            ...
            requestData: {
              ...tokenRequestData,
              client_secret: tokenRequestData.client_secret
                ? `***REDACTED*** (${tokenRequestData.client_secret.length} chars)`
                : undefined,
              code: String(tokenRequestData.code).substring(0, 10) + "...",
              code_verifier: tokenRequestData.code_verifier
                ? `***REDACTED*** (${tokenRequestData.code_verifier.length} chars)`
                : undefined,
            },
            fullRequestBody: formData
              .toString()
              .replace(/client_secret=[^&]*/g, "client_secret=***REDACTED***")
              .replace(/code_verifier=[^&]*/g, "code_verifier=***REDACTED***")
  ...
  async authenticate(
      ...
      if (sessionResult) {
        ...
        if (sessionResult.isExpired && session?.refresh_token) {
          ...
          try {
            ...
            (req as any).expiredSessionToken =
              sessionToken.substring(0, 8) +
              "..." +
          ...
          const tokenSource = req.headers["mcp-session-id"]
            ? "MCP session"
            : "SESSION_TOKEN env";
    ...
    logger.debug(
      {
        ...
        tokenSource: req.cookies?.auth_token
          ? "cookie"
    ...
    if (!token) {
      ...
      if (
        ...
        this.config.enableClientCredentialsFallback
      ) {
        ...
        const clientTokenResult =
          await this.clientCredentialsManager.getAccessToken();
      ...
      if (req.path.startsWith("/mcp")) {
        if (isSessionExpiredFallback) {
          ...
          res.status(401).json({
            ...
            action:
              "Use the start_oauth_login tool to re-authenticate and get a new session.",
          ...
          res.status(401).json({
            ...
            authUrl: "/auth/login",
            message:
              "MCP client needs to authenticate. Visit /auth/login to authenticate.",
          });
        }
        return;
      }
