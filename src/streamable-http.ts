...
export class StreamableHTTPTransport {
  ...
  private getProtocolVersion(req: Request): string {
    ...
    if (
      ...
      headerVersion === FALLBACK_PROTOCOL_VERSION
    ) {
    ...
    logger.warn(
      ...
      "Unsupported protocol version, attempting to handle",
    );
    return headerVersion;
  }
  ...
  private sendSSEEvent(stream: SSEStream, data: any, eventType?: string): void {
    ...
    try {
      ...
      logger.debug(
        { eventId, eventType, sessionId: stream.sessionId },
        "SSE event sent",
      );
    } catch (error) {
      logger.error(
        { error, eventId, sessionId: stream.sessionId },
    ...
    try {
      ...
      try {
        ...
        res
          .status(400)
          .send(
            error instanceof Error ? error.message : "Invalid protocol version",
          );
        return;
      }
      ...
      if (!accept || !accept.includes("text/event-stream")) {
        res
          .status(405)
          .send("Method Not Allowed - Accept: text/event-stream required");
        return;
      }
      ...
      if (lastEventId) {
        ...
        logger.debug(
          { sessionId: session.id, lastEventId },
          "Stream resumption requested",
        );
      }
