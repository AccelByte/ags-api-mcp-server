...
export class StaticTools {
    ...
    if (sessionToken) {
      ...
      if (session && session.refresh_expires_at) {
        ...
        refreshTokenInfo = {
          ...
          refresh_token_expires_at: new Date(session.refresh_expires_at).toISOString(),
   ...
   */
  async startOAuthLogin(args?: any, userContext?: UserContext): Promise<object> {
    ...
    if (!sessionToken) {
      logger.error('No session token found - this should not happen in stdio or HTTP mode');
      
      return {
        success: false,
        error: 'No session token available',
        message: 'Internal error: Session token is missing. This should not happen in stdio or HTTP mode.',
    ...
    const source = userContext?.stdioSessionToken ? 'stdio auto-generated token' : 'MCP session ID';
    logger.debug({
    ...
    if (existingSession?.status === 'authenticated') {
      logger.debug({ sessionToken: sessionToken.substring(0, 8) + '...' }, 'Session already authenticated');
      return {
        success: true,
        ...
        token_expires_at: existingSession.expires_at ? new Date(existingSession.expires_at).toISOString() : undefined,
    ...
    if (!existingSession || existingSession.status === 'expired') {
      ...
      logger.debug({ sessionToken: sessionToken.substring(0, 8) + '...' }, 'Created new session with configured token');
    }
    ...
    if (isStdioMode) {
      ...
      if (!httpServerStatus.available) {
        const errorMsg = httpServerStatus.error || 'HTTP server is not available';
    ...
    if (isStdioMode) {
      ...
      try {
        ...
        return {
          ...
          message: 'Browser window opened for authentication. Please complete the login process.',
          login_url: loginUrl,
          otp_token: otpToken,
          ...
          security_note: 'The URL uses a one-time password token for security. It can only be used once and expires in 10 minutes.',
          note: 'If browser did not open automatically, please check your system settings or manually restart the tool.'
        };
      } catch (error) {
        logger.error({ error, sessionToken: sessionToken.substring(0, 8) + '...' }, 'Failed to launch browser');
        
        return {
          success: false,
          browser_launched: false,
          error: 'Failed to automatically open browser',
          message: 'Browser failed to open automatically. The login URL is available in the server logs.',
          login_url: loginUrl,
          otp_token: otpToken,
          otp_expires_in: 600, // 10 minutes
          instructions: [
          ...
          security_note: 'The URL uses a one-time password token for security. It can only be used once and expires in 10 minutes.',
          troubleshooting: 'If you continue to have issues, ensure your system allows the application to open browsers, or check your default browser settings.'
        };
      }
    } else {
      ...
      return {
        success: true,
        browser_launched: false,
        message: 'HTTP mode detected. Please check the server logs for the authentication URL.',
        login_url: loginUrl,
        otp_token: otpToken,
        otp_expires_in: 600, // 10 minutes
        instructions: [
  ...
  async logout(args?: any, userContext?: UserContext): Promise<object> {
    ...
    if (!sessionToken) {
      ...
      return {
        success: false,
        error: 'No session token available',
        message: 'Cannot logout: No active session found. This should not happen in stdio or HTTP mode.'
      };
    }
    ...
    if (success) {
      ...
      return {
        success: true,
        message: 'Successfully logged out. Your access and refresh tokens have been cleared.',
      ...
      return {
        ...
        message: 'No active session found to logout. You may already be logged out.'
      };
    }
  }
