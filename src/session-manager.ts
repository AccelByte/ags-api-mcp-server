...
export class SessionManager {
  ...
  createSession(baseUrl: string): SessionInfo {
    ...
    return {
      ...
      instructions:
        "Open this URL in your browser to authenticate. After successful login, use the session_token in your MCP client configuration.",
  ...
  getAccessToken(
    sessionToken: string,
  ): { accessToken: string; isExpired: boolean } | null {
  ...
  private cleanup(): void {
    ...
    for (const [token, session] of this.sessions.entries()) {
      ...
      if (
        ...
        now - session.created_at > this.SESSION_TIMEOUT_MS
      ) {
        shouldDelete = true;
      }
      ...
      if (
        session.status === "authenticated" &&
        now - session.last_accessed_at > this.AUTHENTICATED_SESSION_CLEANUP_MS
      ) {
        shouldDelete = true;
      }
