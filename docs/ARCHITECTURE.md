...
Pin persistence lives downstream in athena-facade-api (`.../pinned-queries`); the MCP server stays a stateless proxy (`src/v2/mcp/tools/providers/pinned-queries.ts`). These endpoints are now in the bundled afs spec, so `open_dashboard`/`load_dashboard`/`get_quota_usage` and the save/refresh tools call them directly. Where an environment's facade hasn't deployed them, the call returns an HTTP error: save/refresh surface it and `load_dashboard` falls back to model-held pins with a notice. A server build whose bundled spec predates these endpoints still degrades with a clear `PINNED_QUERIES_UNAVAILABLE` error (the `runApi` lookup miss).
